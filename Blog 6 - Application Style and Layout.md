...
We will choose Bootstrap. Since Bootstrap is used in the default ASP.NET application templates, it is likely that a Microsoft web developer joining the team would have some previous experience with it.

To start with, we will add a reference to Bootstrap via Nuget:

`Install-Package Bootstrap`

This installs a set of CSS, JavaScript and font files into the application Content and Scripts folders. Now we need to ensure that each page in BugTracker.NET properly references these files. We can do this easily by adding the references to the 2 master pages we recently added.

In the head tag, we add a reference to the main CSS file:

`<link href="Content/bootstrap.min.css" rel="stylesheet" />`

Also in the head tab, we will add a couple meta tags. One to ensure the latest rendering engine is used in Internet Explorer. The other to ensure the page width resizes properly on mobile browsers:
...
At the end of the body, we add a reference to the JS file:
...
The very first thing a users sees when using BugTracker.NET is the login screen. We will start our journey by updating the application style here because it is a chance to make a good first impression.

Here is a snapshot of the login page today:

![Original Login Page](Images/LoginOriginal.png)

When I look at this page, there are a few things that stand out to me as making the page look a little dated. First, the text inputs are a little small. The font size could be bigger and some padding would help make the it easier to hit the target on touch devices. The border styling is a on the button and gray section could also be improved. The user and password text inputs are not the same width which makes the page look a little off.
...
We also moved the common Help / Feedback / About links to the footer. This helps to cleanup the main portion of the login page and also allows us to show those links on all the pages of the application.
...
Another added benefit of using Bootstrap is that the layout can be made to change based on the width of the screen. For narrower devices like phones, we can change the layout to be full screen. Previously, BugTracker.NET was using a special mlogin.aspx page for mobile devices. We can delete that page now because our new login page works great on mobile devices.

With minimal effort, we were able to change the look and feel of the login page without changing the behavior in any way. All we did was moved some of the markup and added some small CSS.
...
##Bootstrap Theme
The default Bootstrap theme uses blue to represent primary actions. For now, I would like to use the BugTracker.NET green color for primary buttons. We may decide to change that in the future, but we will start with the existing color scheme for now.

You can customize any aspect of Bootstrap by overriding specific CSS rules. There is also a fully featured [theme builder](http://getbootstrap.com/customize/) that you can use to generate custom themes. For now, we will start by adding a new CSS file called `btnet-bootstrap-theme.css`. In that file, we override the colors for primary buttons:
...
##Main Page
Next, we will tackle the layout and styling of the main application page: bugs.aspx.

![Original main application page](Images/BugsOriginal.png)

We have a lot of work to do on this page. We will break this up into a few pieces. The main menu, the bugs sub-menu, the bugs table.

###Main Menu
The main menu is displayed on almost every page of the application. This is an area of where we can make some changes that will be visible everywhere. My goal with updating the menu is to make it look less cluttered, more standard, and resize better on smaller screens.

The easiest approach will be to use the navbar component from Bootstrap. We will also make some simple changes to make the layout a little more standard. First off, we will move Logoff and Settings items to a dropdown menu under the user name. This is more consistent with other web applications. We can also remove the About and Help links since thos were added to the page footer. This will help to reduce some of the clutter on the main menu.
...
A big benefit of using a Bootstrap navbar is that the navbar is responsive. If the page is resized to be too narrow for the navbar to fit, it will automatically change to a dropdown menu triggered by a menu (a.k.a. hamburger) icon.
...
###Toolbar
Next, we will restyle the main toolbar that is located above the grid. The toolbar provides quick access to add a new bug, print the list of bugs and export to Excel. We can improve the look of this toolbar simply by converting the links to buttons and adding some icons.

As I start to introduce these changes, I notice that some css from btnet-base.css is conflicting with the stying of Bootstrap buttons. We can safely delete this CSS and allow Bootstrap to take over styling of buttons.
...
Likewise, the styling of body, p, td and a several other tags is all causing some inconsistent styling. It is best to keep our CSS to a minimum. We will remove most CSS rules that are setting font sizes or font family.
...
###Grid (Data Tables)
So far, we have made very small changes to the actual code. Mostly we have re-arranged the HTML tags and added some CSS classes. Making changes to the grids is a bigger task, because the grids themselves have a lot of behavior associated with them. We will come back to the grids in the near future. For now, I want to get some high impact UI improvements out to our users without too much delay.
...
It's not easy to design a forms that are both usable and look great. My goal here isn't to going to be to make this form perfect, but I think there are some easy things we can do to improve this form.
...
By applying a the same approach as we did to the main menu, we are able to collapse the bug toolbar on smaller screens and show the Bug Tools dropdown button instead.
...
##Conclusion
With very little effort, we were able to make some high impact improvemnts to the design of a web application that is over a decade old. By using a popular CSS framework, we were able to leverage design expertise from the community without requiring extensive design experience ourselves. As an added bonus, we are  also able to provide a much more pleasant experience for those using mobile devices.
