var BugList = (function () {
    ...
    var updateListOfBugIds = function(currentQuery, newStart, length) {
        var newQuery = {
        ...
        var queryUrl = "api/BugQuery?" + $.param(newQuery);
        return $.get(queryUrl).done(function(d) {
            setQueryParams(newQuery);
            saveCurrentBugList(d);
        });
    }

    //Returns a promise that resolves with the next/previous bugid, the count of all bugs and the index of the current bug:
    // e.g. {  bugIndex: 10, count: 100, previousBugId: 1042, nextBugId: 514 }
    //Returns null if the bugId is not found in the current list
    //previousBugId = null bugId is at the beginning of the list
    //nextBugId = null if bugId is at the end of the list
    var getBugDetails = function (bugId) {
        var currentBugList = getCurrentBugList();
        var currentQuery = getQueryParams();
        var deferred = $.Deferred();
        var bugListExists = currentBugList && currentBugList.bugIds && currentQuery;
        if (bugListExists) {
            var isReady = true;
            var indexOfBugInCurrentPage = currentBugList.bugIds.indexOf(bugId);
            if (indexOfBugInCurrentPage >= 0) {
                var indexOfBugInOverallList = indexOfBugInCurrentPage + currentQuery.start + 1;

                var previousBugId = null;
                var bugListPageSize = 25;

                if (indexOfBugInCurrentPage > 0) {
                    previousBugId = currentBugList.bugIds[indexOfBugInCurrentPage - 1];
                } else if (indexOfBugInOverallList > 1) {
                    isReady = false;
                    //Shift the paged list of bugids to the left
                    var startShiftedLeft = Math.max(indexOfBugInOverallList - bugListPageSize, 0);
                    updateListOfBugIds(currentQuery, startShiftedLeft, bugListPageSize)
                        .done(function () {
                            getBugDetails(bugId).done(function (shiftedDetails) {
                                deferred.resolve(shiftedDetails);
                            }).fail(function () {
                ...
                var nextBugId = null;
                if (indexOfBugInCurrentPage < currentBugList.bugIds.length - 1) {
                    nextBugId = currentBugList.bugIds[indexOfBugInCurrentPage + 1];
                } else if (indexOfBugInOverallList < currentBugList.recordsFiltered) {
                    isReady = false;
                    //Shift the paged list of bugids to the right
                    var startShiftedRight = Math.max(indexOfBugInOverallList - 5, 0);
                    updateListOfBugIds(currentQuery, startShiftedRight, bugListPageSize).done(function (d) {   
                        getBugDetails(bugId).done(function(shiftedDetails) {
                            deferred.resolve(shiftedDetails);
                        }).fail(function() {
                            deferred.resolve(null);
                ...
                if (isReady) {
                    deferred.resolve({
                        bugIndex: indexOfBugInOverallList,
                        count: currentBugList.recordsFiltered,
                        previousBugId: previousBugId,
                        nextBugId: nextBugId
                    });
                }
    ...
    return {
        ...
        saveCurrentBugList: saveCurrentBugList,
